...
for i in range(len(files_in)):
    ...
    poly_out_pth=os.path.join(shape_dir_common_roi,  file_basename+'_lakes.shp') # shape_dir shape_dir_common_roi #Modify here if making lake polys for full or #sub-roi'/mnt/f/PAD2019/classification_training/PixelClassifier/Test35/shp/padelE_36000_19059_003_190904_L090_CX_01_LUT-Freeman_cls_poly.shp'
    ouput_raster_pth = os.path.join(output_raster_dir_common_roi, file_basename + '_brn.tif') # brn=burned #sub-roi # output_raster_dir output_raster_dir_common_roi
    ...
    roi=gpd.read_file(roi_sub_pth) # roi_pth roi_sub_pth #sub-roi
    roi = roi[roi.Region == region_lookup_sub_roi[file_basename]] #sub-roi
    if len(roi) == 0:

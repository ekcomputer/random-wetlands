...
% From Matlab File Exchange CaptureFigVid: https://www.mathworks.com/matlabcentral/fileexchange/41093-create-video-of-rotating-3d-plot?focused=3781471&tab=function
% by Alan Jennings, modified by Ethan Kyzivat
% TODO: put parms as function calls
...
% end
f=getframe(gcf);
f.cdata=imresize(f.cdata, 0.25);
[im,map] = rgb2ind(f.cdata,256,'nodither');
% [im,map] = imresize(im, map, 0.25);
im(1,1,1,size(ViewZ,1)) = 0;
for kathy=1:size(ViewZ,1)
    view(ViewZ(kathy,:)); 
    f = getframe(gcf);
    f.cdata=imresize(f.cdata, 0.25);
    im(:,:,1,kathy) = rgb2ind(f.cdata,map,'nodither');  
